#新闻相关的缓存方法：新闻分类的读取跟写入
#key -value
from typing import List, Dict, Any, Optional

from config.cache_conf import get_json_cache, set_cache

CATEGORISE_KEY = "news:categories"
NEWS_LIST_PREFIX = "news:list:"

#获取新闻分类缓存
async def get_cached_categories():
    return await get_json_cache(CATEGORISE_KEY)

#写入新闻分类缓存： 缓存的数据，过期的时间 数据越稳定，缓存越持久
#避免所有的key同时过期   引起缓存雪崩
async def set_cached_categories(data: List[Dict[str, Any]] , expire: int = 7200):
    return await set_cache(CATEGORISE_KEY , data , expire)
